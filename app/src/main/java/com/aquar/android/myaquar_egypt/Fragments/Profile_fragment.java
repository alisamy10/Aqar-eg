...
import com.aquar.android.myaquar_egypt.Model.Login.userObjPOJO;
...
//    TextView location;
    ...
    private void dataCheck() {
        Gson gson = new Gson();
        userObjPOJO userPOJO = gson.fromJson(mySharedPreference.getUserOBJ(), userObjPOJO.class);
        try {
            if (!Objects.equals(userPOJO.getEmail(), null)) {
                Glide.with(getActivity()).load(userPOJO.getImage()).into(profile_photo);
                user_name.setText(userPOJO.getName());
                username.setText(userPOJO.getName());
                job_title.setText(userPOJO.getJob_title());
                email.setText(userPOJO.getEmail());
                mobile.setText(userPOJO.getPhone());
                job_titleBar.setText(userPOJO.getJob_title());
            }
